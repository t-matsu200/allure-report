...
# login
curl -sk -X POST "${ALLURE_SERVER}/allure-docker-service/login" -H  "accept: */*" -H  "Content-Type: application/json" -d "{\"username\": \"${ALLURE_USER}\",\"password\":\"${ALLURE_PASS}\"}" -ik | tr -d '\r' | tee /tmp/login.txt
...
if [ $STATUS_CD -ne 0 ]; then
  echo "Failed to send report files?"
  exit $STATUS_CD
fi

echo "----------------GENERATE-REPORT-----------------"

RES=$(curl -X GET "${ALLURE_SERVER}/allure-docker-service/generate-report?project_id=$PROJECT_ID" -H "X-CSRF-TOKEN: $CSRF_TOKEN" --cookie "access_token_cookie=${ACCESS_TOKEN}" $FILES)

STATUS_CD=$?
if [ $STATUS_CD -ne 0 ]; then
  echo "Failed to send generate reports?"
  exit $STATUS_CD
fi

ALLURE_REPORT_URL=$(echo $RES | jq '.data.report_url' | sed 's|\"||g')
